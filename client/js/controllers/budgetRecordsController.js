  ...
  constructor($scope) {
    ...
    let calculateTodays = () => {
      ...
      _.map(vm.budgetRecords, num => {
        if (num.expenseType) {
          vm.todays.total += isNaN(parseFloat(num.amount))
            ? 0
            : parseFloat(num.amount);
        }
      });
    };
    ...
    vm.addNewRow = index => {
      ...
      if (
        vm.budgetRecords[vm.budgetRecords.length - 1].expenseType != 0 ||
...
angular
  .module("moneyManager")
  .controller("budgetRecordsController", budgetRecordsController)
