export default class expenseRecordsController {
  constructor($rootScope, $scope) {
    ...
    let calculateTodays = () => {
      ...
      _.map(vm.expensesRecords, num => {
        if (num.expenseType) {
          ...
          if (num.spendingsType == 1) vm.todays.earnings += amt;
          else if (num.spendingsType == 2) vm.todays.spendings += amt;
        }
      });
    };
    ...
    vm.addNewRow = index => {
      ...
      if (
        vm.expensesRecords[vm.expensesRecords.length - 1].expenseType != 0 ||
    ...
    $scope.$watch("erc.getExpenseValidation", newValue => {
      if (newValue && newValue.validAll !== 0) vm.dateChosen = true;
    });
...
angular
  .module("moneyManager")
  .controller("expenseRecordsController", expenseRecordsController)
