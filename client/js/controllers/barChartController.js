export default class barChartController {
  constructor() {
    ...
    vm.$onChanges = function() {
      if (vm.barData) {
        d3.select(".yearlyBarChart")
          .select("svg")
          .remove();
        d3.select(".monthlyBarChart")
          .select("svg")
          .remove();
        d3.select(".dailyBarChart")
          .select("svg")
          .remove();
        ...
        let barChartStart = cate => {
          if (
            angular.copy(
              d3.select("." + cate + "BarChart").select("svg")[0][0]
            ) != null
          )
            d3.select("." + cate + "BarChart")
              .select("svg")
              .remove();
          ...
          var y0 = d3.scale
            .ordinal()
            .rangeBands([0, height])
            .domain(_.pluck(vm.barData[cate], "time"));
          ...
          function ready(data) {
            ...
            //Appends first bar
            var bars = categoryGroup
              .append("rect")
              .attr("width", function(d) {
                return xScale(d.amount);
              })
              .attr("height", y0.rangeBand() / 1.5)
              .attr("class", vm.barColorClass[cate])
